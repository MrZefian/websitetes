  ...
  parse(client, message) {
    if (this.devOnly && !client.config.owners.includes(message.author.id)) {
      message.channel.send(
        `${message.author} Sorry, but you're not my **\`DEVELOPERS\`**.`
      );
      return false;
    }
    ...
    if (this.clientPermission.length) {
      ...
      if (perms.length) {
        message.channel.send(
          `${message.author} Sorry, but you're not our Staff(s).`
        );
        return false;
      }
    }
      ...
      if (
        ...
        message.channel
          .send(
            `${
              message.author
            }, You can use this command again in \`${estimatedTime / 1000}s\`**`
          )
