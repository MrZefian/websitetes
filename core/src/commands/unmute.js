...
✅ Successfully **\`UN-MUTED\` ${mention.user.tag}**, Check <#${config.servers.ModerationID}> for more details.
`);

    let muteEmbed = new RichEmbed()
      .setAuthor("UN-MUTED.", message.author.avatarURL)
      .setColor("RED")
    ...
    if (message.author.bot === false) {
      let dmEmbed = new RichEmbed()
        ...
        .setDescription(
          `
You have been **UN-MUTED** on the Server, because:
\`\`\`${reason}\`\`\`
`
        )
        .addField(
...
`
      ...
      mention
        .send(dmEmbed)
        .then(() => mention.removeRole(role))
        .catch(err => console.info(`[UN-MUTED ERROR]: ${err}`));
    }
  }
};
